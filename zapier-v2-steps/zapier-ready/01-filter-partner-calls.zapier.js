 ...
 * "Run Javascript" editor for Step 1.
 *
 * Derived from ../01-filter-partner-calls.js (the source of truth / Node-testable
 * version — edit and test logic changes there, then re-generate this file; do not
 * hand-edit the two copies separately). Only two things differ from the source file:
 *
 *   1. The Apollo API key is read from `inputData.apolloApiKey` instead of
 *      `process.env.APOLLO_API_KEY`. In the Zap step's Input Data panel, map
 *      `apolloApiKey` to wherever the secret actually lives (a Storage by Zapier
 *      value, an upstream step, etc.) — Zapier Code steps have NO access to this
 *      repo's local .env, and this design uses an explicit inputData field rather
 *      than Zapier's separate "Environment Variables" feature.
 *   2. The step ends with `return {...}` instead of assigning to the bare `output`
 *      global — `return` is the contract Zapier's Code-step sandbox expects.
 *
 * No other logic changed. Confirmed clean for Zapier's sandbox: no module.exports,
 * no require()/import, no npm dependency — the Levenshtein edit-distance check below
 * is hand-rolled plain JS, nothing to inline.
 *
 * ---------------------------------------------------------------------------------------
 ...
 *   inputData: apolloApiKey (map this field in the Zap step's Input Data panel — see
