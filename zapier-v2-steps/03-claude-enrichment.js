 ...
 *   Requires Zapier Environment Variables: ANTHROPIC_API_KEY
 ...
 * resolveCompanies() and docs/zapier-v2-setup.md for the full explanation. That step
 * loops over the parsed array internally in plain JavaScript, not via a second
...
if (!apiKey) {
  throw new Error("ANTHROPIC_API_KEY is not set in this Zap's environment variables");
}
...
output = {
  ...
  // 04-match-and-write-companies.js and docs/zapier-v2-setup.md. Zapier's "Step
  // Output {...}" data-picker inserts an ENTIRE step's output as one stringified
  // object when mapped into a downstream Input Data field, not just one nested
  // array — confirmed live, mapping Step Output to `companies` produced the whole
  // { companies, partner, startTime, companyCount } object stringified, not the
  // array alone, and Step 4 failed with "No companies array in inputData". Map THIS
  // field (a plain string pill) to Step 4's companiesJson input instead.
